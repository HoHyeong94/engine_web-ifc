...
export function generateStruct(entity: Entity, classBuffer: Array<string>, types: Type[], crcTable: any) {
    classBuffer.push(`    #[repr(C)]`);
    classBuffer.push(`    #[derive(Debug, Clone)]`)
    classBuffer.push(`    pub struct ${entity.name}<'a> {`);
    classBuffer.push(`        type_: u32,`);

    const inverseProps: string[] = [];
    const inversePropsName: string[] = [];
    const derivedProps: string[] = [];
    const derivePropsName: string[] = [];
    ...
    entity.inverseProps.forEach((prop) => {
        ...
        classBuffer.push(`        ${propName}: ${type},`);
    });
    ...
    classBuffer.push(`    impl<'a> ${entity.name}<'a> {`);
    // classBuffer.push(`        pub fn from(${inverseProps.length > 0 ? inverseProps.join(", ") + ", " : ""}${derivedProps.join(", ")}) -> Self {`);
    classBuffer.push(`        pub fn from(${derivedProps.join(", ")}) -> Self {`);
    classBuffer.push(`            ${entity.name} {`)
    classBuffer.push(`                type_: ${crc32(entity.name.toUpperCase(), crcTable)},`);

    inversePropsName.map((n) => {
        classBuffer.push(`                ${n}: None,`);
    })
    derivePropsName.map((n) => {
        classBuffer.push(`                ${n}: ${n},`);
    });
    classBuffer.push(`            }`);
    classBuffer.push(`        }`);

    classBuffer.push(`        pub fn isifcroot() -> bool {`);
    //isifcRoot
    if (ifcRootProps === 4) {
        classBuffer.push(`            true`);
    } else {
        classBuffer.push(`            false`);
    }
    classBuffer.push(`        }`)
    ...
    if (!entity.parent) {
        ...
        classBuffer.push(`            IfcLineObject::from(None)`);
        classBuffer.push(`        }`);
        classBuffer.push(`    }`);
    } else {
        classBuffer.push(`    impl<'a> Into<${entity.parent}<'a>> for ${entity.name}<'a> {`);
        classBuffer.push(`        fn into(self) -> ${entity.parent}<'a> {`);
        var nonLocalProps = entity.derivedProps.filter(n => !entity.props.includes(n));
        ...
        if (nonLocalProps.length == 0) {
            classBuffer.push(`            ${entity.parent}::from()`);
        } else if (isExistNew) {
            classBuffer.push(`            todo!();`);
        }
        else {
            classBuffer.push(`            ${entity.parent}::from(${nonLocalProps.map((p) => `self.${generateSuperAssignment(p, entity.ifcDerivedProps, types).toLowerCase()}`).join(", ")})`);
        }
        classBuffer.push(`        }`);
        classBuffer.push(`    }`);
    }
